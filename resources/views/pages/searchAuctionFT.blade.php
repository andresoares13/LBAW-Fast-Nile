...
@section('content')
        @if (count($auctions)!=0)
        <p id="pagePara">
          Auctions containing: "{{$query}}" (Full text search)
        </p>
        ...
        @else
        <p id="pagePara">
          There are no auctions containing: "{{$query}}"
        </p>
        <p>
        <form id="formSearch" class="None" action="/search/auction"  method="get" role="search">
          <input type="search" id="query" name="q"
          placeholder="Search for an Auction..."
          aria-label="Search through site content" >
          <button type='submit'>
